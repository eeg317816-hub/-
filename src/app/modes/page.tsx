...
export default function ModesPage() {
  ...
  useEffect(() => {
    ...
    const s = getSession();
    if (!s.playerId || !getTerminalSessionId()) {
    ...
    <GameShell scene="hud">
      <div className="mx-auto min-h-screen max-w-xl px-6 py-10">
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="hud-frame p-6"
        >
          <div className="font-display text-2xl text-[#ff3b45]">职业 APM 测试</div>
          <p className="mt-3 text-sm leading-relaxed text-[#bbb]">
            键鼠混合挑战。正确操作加分，失误扣分断连。综合分进入今日榜 / 总榜。
          </p>
          {remain != null && (
            <p className="mt-4 font-tech text-sm text-[#ff6b73]">
        ...
        <button
          ...
          disabled={loading}
          onClick={start}
          className="btn-game mt-8 w-full rounded-md py-3 text-lg font-semibold"
        >
          {loading ? "准备中…" : "开始挑战"}
        </button>
